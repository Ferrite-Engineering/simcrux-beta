...
> **SimCrux** is a simulation regression manager for open-source HDL simulators —
> orchestrate Icarus Verilog, Verilator, GHDL, and cocotb runs in parallel, watch
> results land in a live dashboard, drill into a failing test's log, track trends
> across runs, and jump straight into the waveform. Linux, macOS, and Windows,
> plus a read-only web dashboard for exported results.

**This repository contains no application source code.** During the public beta
the SimCrux source is closed; this repo exists purely as the public meeting
point for the beta:
...
- 🧪 **[Submit a test fixture](../../issues/new?template=fixture_submission.yml)** —
  hand us a regression project that SimCrux runs or reports wrong and we'll fold
  it into the suite.
...
1. In SimCrux, open **Help → Submit Issue** (also in the command palette and the
   About box).
...
3. Hit **Submit**. SimCrux copies a formatted report to your clipboard and opens
   a pre-filled new-issue form **in this repository**. Paste if needed, drag in
   the screenshot, and submit.

No private file contents, test names, simulator output, or file paths are
...
**Simulator bugs deserve one extra line:** tell us which simulator and its exact
version (`iverilog -V`, `verilator --version`, `ghdl --version`, or
`cocotb-config --version`). SimCrux drives these as subprocesses, so the version
...
When SimCrux opens its source post-beta, the canonical repository becomes
...
*SimCrux is built by [Ferrite Engineering](https://ferriteengineering.com).
